...
- ✅ **DONE 2026-08-10 (S402, Claude) — backend track recording shipped and verified on CD.** See the Changelog entry for what was built and how it was tested. The spec below is kept as the record of *why*.

- 🔴 **NEW — stop detection never fires on tracks from a speed-reporting GPS (found while testing the recorder, 2026-08-10, NOT introduced by it).**
  **Symptom:** a deliberate 80-second halt in a synthetic drive produced `stops: []`. The Debrief stop timeline is therefore effectively non-functional on real recordings.
  **Root cause (isolated by running the shipping `_detect_stops` against the recorded points, with and without the `speed` field):** the minimum-movement gate collapses a halt into a single point whose **timestamp is refreshed** while the vehicle sits — but its **`speed` value is left at whatever it was when the point was captured** (e.g. 50 km/h, recorded while still moving). `_seg_speed_kmh()` prefers the stored `speed` fields over distance÷time, so the segment spanning the halt averages `(50+0)/2 = 25 km/h` and never trips `STOP_KMH = 1.0`. Measured on the test track: `idx 10→11, dt=84 s, dist=13.9 m` → **25.00 km/h** with speed present, **0.59 km/h** with it stripped. Stripping `speed` from the same 52 points yields the 2 correct stops (85 s and 181 s).
  **Why it was never noticed:** older tracks from receivers that don't report RMC/VTG speed have no `speed` field, so they fall through to the distance÷time path and *do* detect stops. The u-blox dongle reports speed, so newer/better data silently detects fewer stops than worse data. (The 2026-07-17 verification that "passed" used a synthetic track whose parked points carried speed 0 throughout, so it never exercised this path.)
  **Proposed fix (not applied — changes recorded data semantics, needs Filip's go-ahead):** while stationary, keep **two** at-rest points instead of one — halt-start and halt-end, both with the *current* near-zero speed — and refresh only the second one's timestamp. The segment between them then reads ~0 km/h for the full halt duration and the stop is detected, while still adding no fake distance. ~6 lines in `_rec_consider()`.
  **Note:** this only fixes *future* recordings. Existing tracks would need a one-off re-detect that ignores the stored speed field.

- **Backend track recording — original spec, now IMPLEMENTED (kept for the reasoning). Sharpened 2026-08-10 (Claude, code-verified):**
  **Mechanism (why it failed):** `gps.py` already owns the device server-side and Flask serves `/api/gps` — that half is fine. But the *recording* lives in the page: the browser polls `/api/gps` on a timer and, inside that same poll handler (`app.js:5252`), calls `captureGpsPoint()`, which filters the point and pushes it onto a **JS array in the tab's memory**, mirrored to `localStorage` (`ops_toc_active_track`). On Save the browser POSTs the whole array to `/api/tracks`. **So the browser's poll cycle IS the recording loop** — no rendered page (tab closed, browser crash, device asleep, app switched) = no points.
...
**2026-08-11** — [Claude, S404] 🔴 **`map_app.db` had NO backup — 79 MB of irreplaceable field data on a device that travels.** Found sweeping the shared-database layer. Not in Syncthing (`~/maps` sits outside `Projects/` and `Claude-Home/`), no cron, no timer; the only copy in existence was the one made hours earlier before the stop back-fill. Contents: **62 GPS tracks / 165,961 points** (months of real driving), every marker, 105 stops. Same class as the Mesh-Torry game DB — arguably worse, because CD is **portable** and its NVMe is a single point of failure.
**Fix, reusing the chain proven twice today:** `~/.local/bin/backup-opstoc-db.sh` → `iterdump()` → gzip (**79 MB → 2.8 MB in 2.3 s**) → 7 kept locally → rsync to M-H `/mnt/nextcloud_data/opstoc-db-backups/` → M-H's 03:00 Hetzner leg. No new credentials, no new target.
⚠️ **Deliberately a user systemd timer with `Persistent=true`, not cron.** CD is off most of the time, so a plain daily schedule would simply never fire; `Persistent=true` runs the missed job shortly after the next boot. (Also: no passwordless sudo on CD, so a system unit was not an option anyway.) And a **dump, not a file copy** — `map_app.db` is in `delete` journal mode, where copying a live file can capture a torn database.
**Verified:** ran via systemd exactly as the timer will → exit 0, 2.8 MB, landed on M-H, `gzip -t` passed, and **rebuilt into a scratch database**: 62 tracks, 3 markers, 165,961 points. That last figure reconciles exactly with 167,011 minus the 1,050-point demo track deleted earlier — the dump is neither stale nor truncated.

**2026-08-11** — [Claude, S404] **`json_array_length('')` could have 500'd the entire track list.** The restore check threw `malformed JSON` on `stops_json`, which exposed a fragility in the summary endpoint added earlier the same day: `json_array_length(points_json)` raises on an empty string, and that would fail the **whole** `/api/tracks` query rather than one row. **The empty case is real in this schema, not hypothetical — 17 rows already hold `''` in `stops_json`** (the column default). Guarded with `CASE WHEN json_valid(points_json) THEN … ELSE 0 END`.
**Armed-tested:** injected a track with `points_json = ''` → list returns **200** with the bad row reporting `point_count = 0`, instead of a 500. ⚠️ The first run of that test *did* 500 — because the service had been restarted before Syncthing delivered the fix. Re-run after confirming the bytes on CD. **An ambiguous armed test is worth re-running, not interpreting.**

**2026-08-11** — [Claude, S404] 🔴 **Folder names with an apostrophe silently broke their own button — and were an injection vector.** `app.js` built inline handlers as `onclick="toggleMarkerFolder('${esc(name)}')"`. **`esc()` alone is not safe in that position:** the browser HTML-decodes the attribute *before* parsing the JS, so `&#39;` becomes a real `'` and closes the string. Demonstrated by simulating decode-then-parse: `"Mum's place"` → **SyntaxError**, so the collapse/expand button does nothing; `"'); alert(1); //"` → **parses and executes**. `toc.js` had this right at all 13 of its sites via `jsAttr()` (backslash-escape for JS *then* entity-escape for HTML); `app.js` had it wrong at **4** — marker folders (L1543/L1561) and track folders (L2530/L2548).
⚠️ **I made two of those reachable earlier the same day** by adding the marker-folder input that `a3c38dc` never shipped. Before that there was no way to set a marker folder at all.
Fixed: `jsSafe()`/`jsAttr()` now live **once**, in `app.js` beside `esc()` (toc.js loads second and already depended on `esc()`, so its duplicates were removed). All 4 sites converted; **0 `esc()`-in-quoted-handler sites remain in either file**. Re-verified with the same decode-then-parse simulation: `Mum's place`, `Nik's`, `O'Brien` all parse correctly now, and the injection payload becomes an inert single argument.
*Swept clean in the same pass:* the OverMesh overlay handlers were already safe — `key` is stripped to `[a-zA-Z0-9_-]`, `network` is `mc`/`mt`, and popup payloads use `encodeURIComponent(JSON.stringify(...))`. Every render of user/GPX text into HTML uses `esc()`. `appDialog` uses `textContent`, so confirm dialogs are not an HTML sink.

**2026-08-11** — [Claude, S404] **A failed delete looked like a dead button.** `deleteTrack()` and `deleteDrawing()` are `async` and called from inline `onclick`, so nothing awaits them — and neither guarded its `api(..., DELETE)` call. If the delete failed (network drop, 500, service restarted mid-action) the rejection went unhandled, **`loadTracks()`/`loadDrawings()` never ran, the item stayed on screen, and no error appeared** — reading exactly like "the button doesn't work", which is the same symptom that turned out to be the dead Debrief button. Both now catch and `appAlert`, matching the pattern `saveTrack()` already used.
*Audited all 17 async functions reachable from inline handlers.* Two still flag as "unguarded" — `renderTrackColored` and `toggleTrackVisibility`, both written earlier today — but that is a **false positive**: each has exactly one `await`, `ensureTrackPoints()`, whose own await is inside a try/catch returning `null`. Verified by reading every await rather than trusting the scanner.

**2026-08-11** — [Claude, S404] **All localStorage writes are now quota-safe — and the fix itself nearly shipped an infinite recursion.**
Closed the gap flagged in the previous entry: `safeSetItem()` in `app.js` (with `tocSetItem()` in `toc.js`, which loads second and defers to it) wraps every write. **34 + 4 call sites converted**; the only raw `localStorage.setItem` left in either file is the one inside each helper. A full store — or Safari private mode, where `setItem` throws even when empty — now logs a warning and returns `false` instead of crashing whatever happened to be saving.
⚠️ **The near-miss is the point.** The bulk regex `localStorage.setItem(` → `safeSetItem(` also rewrote the call **inside the helper**, making it recurse infinitely — every write would have blown the stack. The first armed test reported a `RangeError` and I initially dismissed it as a broken harness. **It was real signal: `Maximum call stack size exceeded`.** Caught only because the test was re-run properly against the file's actual bytes (brace-matched extraction) rather than a hand-copy. **Lesson: when a test disagrees with the code you just wrote, suspect the code first.** Re-verified after the fix: full store → returns `false` + warns, normal store → returns `true`, no recursion.

**2026-08-11** — [Claude, S404] **Checklist backups grew forever and nothing could read them.** `checklistBackupCurrent()` wrote a **new timestamped key** per call holding a full copy of every checklist — and `CHECKLIST_BACKUP_PREFIX` appears exactly twice in `app.js`: the constant and that write. **No restore path, no UI, no pruning.** So every checklist import permanently deposited unreadable dead weight in localStorage. Now keeps the **3 most recent** (lexicographic sort == chronological, since the keys are timestamps) and the write is wrapped so a full store degrades to a console warning instead of throwing. Prune logic unit-checked: 5 imports leave the 3 newest.
⚠️ **Wider gap noted, not fixed: there is no `QuotaExceededError` handling anywhere** — 24 of ~34 `localStorage.setItem` calls are unguarded. Impact is low today (only this one grew without bound), but if the store ever fills, the failure is a throw from an unrelated place — hard to diagnose. A `safeSetItem()` helper across all sites would close it; that's a broader refactor and wasn't done unasked.
*Also swept clean this round:* the tile downloader is genuinely solid — writes to `.part`, validates a resumed partial with `is_readable_mbtiles()` and discards it if corrupt, unlinks on cancel, and finishes with `tmp_path.replace(path)` (**atomic rename**); job state is properly guarded by `download_condition`.
...
**2026-08-11** — [Claude, S404] **Marker folders finished (Filip's call: finish, not drop) + a coordinate-validation gap closed.**
*Marker folders.* `a3c38dc` (2026-06-23) had shipped the **frontend grouping, collapse state and localStorage** for marker folders but **neither the DB column nor the input** — which is what broke Add/Edit Marker outright for ~7 weeks. Completed all four layers: `ALTER TABLE markers ADD COLUMN folder` (mirroring the tracks migration pattern, in the same try/except so it is idempotent), `_marker_row()` exposes `folder`, create **and** update accept it (`_clean_text(..., 80)`), and the dialog gained a free-text input with a `marker-folder-datalist`. Markers use the **same `Parent/Sub` convention as tracks** — the grouping code already called `_parseTrackFolder()`, so nothing new was invented.
*Verified live:* column present; create with `Work/Sites` round-trips through GET; PUT changes it to `Home/Garden`; PUT with `""` clears it; **pre-existing markers all return `folder: ""`** rather than null, so the list groups them under "no folder" as before. Test marker deleted — 3 markers before and after.
*Coordinate validation (sweep find).* Nothing anywhere checked that a latitude or longitude was **on Earth** — `_float()` rejected NaN/Inf but `lat=999` was stored happily, which would break `fitBounds` and place a marker somewhere impossible. The realistic vector is a **malformed GPX import**, not a typo. Added `_coord()` (range-checked: ±90 / ±180) and swapped **all 19** lat/lon parse sites to it — markers, track points, drawings, GPX import, geojson import. **Armed-tested:** `lat=999` → 400 `Invalid lat`, `lon=999` → 400 `Invalid lon`, valid → 200.
...
· **Add/Edit Marker** — `el("marker-folder")`, which **does not exist in `index.html` at all**. `a3c38dc` (2026-06-23) "Add folder/sub-folder support for markers" added the folder JS, the grouping UI and the localStorage state, but **never the input — and markers have no `folder` column in the DB either**. So marker add/edit had been throwing for ~7 weeks. Made null-safe rather than deleting the half-built feature; **whether to finish or drop marker folders is a decision for Filip** (finishing needs a template input *and* a migration).
**Swept the whole file for the class:** 183 distinct `el("…")` ids checked against the template — 7 missing, of which `track-color-legend`/`track-save-custom-color` are created dynamically and `markers-wrap`/`markers-menu` already use optional chaining. After the fixes: **0 unguarded references to a missing id.**

**2026-08-11** — [Claude, S404] **Follow-up: the summary endpoint was still parsing every point.** Filip reported it felt slow. Measured: **32 KB took 0.46 s while a single 753 KB track took 0.066 s** — 7× the data, 7× faster. Cause: `_track_row()` still ran `json.loads(points_json)` for every track purely to compute `point_count`, then discarded the array. **I had removed the transfer cost but kept the parse cost** — 167k points parsed on every list request. Fixed: summary mode selects every column *except* `points_json` and gets the count from SQLite's `json_array_length()`. **0.46 s → 0.13 s**, counts still exactly 167,011, stops still 105, `?points=1` unaffected.
*Remaining 0.13 s is SQLite parsing the JSON to count* (~0.08 s of it). Removing that needs a stored `point_count` column maintained on write — a migration, not yet done. For context the whole rest of the load path is 3–30 ms per endpoint, tiles included, and the box is idle at load 0.18.

**2026-08-11** — [Claude, S404] **Sweep of my own payload change — two defects found and fixed.**
(1) **`renderTrackColored()` — the lazy-load was a no-op.** I added `await ensureTrackPoints(trackId)` but the next line read `_trackCache[trackId]`, and `ensureTrackPoints` populates `state.tracks`, not `_trackCache`. Colouring a track by speed/altitude whose points had never been fetched silently bailed. Now takes the ensure's return value, falling back to the cache.
(2) 🔴 **Silent failure reported as success.** `toggleTrackVisibility()` set `trackVisible = true` *before* awaiting the points. If the fetch failed, `renderTrack()` returned early, no layer was created — and the list rendered the track as **shown** while nothing was on the map. Same shape as the Mesh-Torry `flush()` bug (a loss reported as a success). Now reverts the flag, re-renders the list and says why — distinguishing "too few points to draw" from "couldn't load, check the connection", reading the count from the **summary** we already hold, because the ensure returns `null` on failure and using it would misreport a network error as an empty track.
*Also verified clean in the same sweep:* every `_rec_consider()` call site holds `_rec_lock` (both of them), `_seg_speed_kmh` guards its division with `dt = max(..., 1)`, and `trackStats()` returns `null` rather than throwing on a summary track.

**2026-08-11** — [Claude, S404] **`/api/tracks` went from 21.5 MB to 32 KB — a 634× cut on every page load.**
*Found in a bug sweep.* The list endpoint returned every track **with all its points**: 167,011 points across 63 tracks, **21.5 MB**, of which the metadata the list actually uses was **33.6 KB — points were 99.85% of the payload**. Worse than the download: `loadTracks()` called `renderTrack()` for *every* track, building Leaflet geometry for all 167k points and then only adding a layer to the map if that track was toggled visible. Most of that work was discarded on every load — on a Rock 5B and the HD 5" Lite UI. And it grows monotonically: ~4,000 points per drive, #31 alone is 6,235.
*Fix.* `_track_row()` gained `include_points`; every track now always carries **`point_count`**. `GET /api/tracks` returns a **summary by default**, with **`?points=1`** preserved for export/backup callers. Added **`GET /api/tracks/<id>`** — that route did not exist (it 405'd), which is precisely why the frontend fetched the entire list just to read one track (`showTrackDebrief` did exactly that). Frontend gained `ensureTrackPoints(id)`, and points are now fetched lazily on first show / fly-to / colour-by-speed; the list uses `point_count`; `toc.js`'s three count usages updated.
*Measured on CD:* **20,475,398 → 32,297 bytes**. `?points=1` still 20.4 MB. `/api/tracks/31` → 200 (753 KB) where it was 405; `/api/tracks/9999` → 404. Summary `sum(point_count)` = **167,011**, exactly matching the original total. **Exports unaffected — `/api/export/gpx` still emits all 167,011 `<trkpt>` entries.**
...
✅ **Historical tracks back-filled the same day (Filip: "sure, go on").** A one-off re-detect with `speed` **stripped** forces the distance÷time fallback — correct for a parked vehicle, and exactly how tracks from non-speed-reporting receivers are already handled. Ran the **real shipping `_detect_stops`** extracted from `app.py` source, not a reimplementation. **Result: 40 tracks gained stops; 41 of 63 now have them, 105 stops total** (was 1 track / the seeded demo).
*Safety, all deliberate:* DB copied to `~/maps/map_app.db.bak-2026-08-11-pre-stop-redetect` first; `ops-toc` stopped during the write so there were no concurrent writers; **only tracks with zero stops were touched**, so annotated stops could never be overwritten (0 were annotated anyway); and **`updated_at` was left alone** — the track list is ordered by it and a bulk re-detect must not reshuffle Filip's list.
*Validated before applying, not after:* the risk with stripping speed is that **sparse sampling fakes a halt** (one long gap with little movement). Spot-checked the four largest — tracks 8/30/31/2 — and every one is densely sampled inside the run (median 1–12 s intervals, 16–4456 points clustered in one place). Real halts. The 22 tracks that still show none are genuine continuous drives.
...
*The trap was avoided.* The four capture filters (4-satellite minimum, interval throttle, multipath rejection, minimum-movement gate) were **moved** to `_rec_consider()` in Python and removed from JS — one implementation, as the spec demanded. The frontend no longer computes the live track's distance either; it displays the server's `distance_m`. `_insert_track()` was factored out so `POST /api/tracks` and the recorder's save share one write path (identical stop detection, distance, defaults).
*Crash/restart survival.* The buffer is written atomically to `~/maps/active_track.json` (flushed at most every 5 s when dirty) and reloaded at import, so a service restart or crash resumes mid-track. Worst case loss ≈5 s. Save/discard removes the file. `/stop` deliberately **keeps** the buffer — the Track button then reads `Save (N)` so a stopped-but-unsaved track can't be silently lost.
*Deliberate behaviour choices.* The recorder keeps running while the save dialog is open (matches the old semantics — points captured during the dialog belong to the track); cancelling the dialog leaves it recording. `start` refuses with 409 if a track is already active or an unsaved buffer exists, and refuses without a GPS fix.
*How it was tested (indoors, no GPS fix available).* Built a fake OM GPS source on CD serving `/api/settings/gps` and drove OPS-TOC's existing proxy mode at it — same `gps_state` the recorder reads, no root needed (a pty was tried first but `gps_start()` rejects it: `port_present()` only accepts devices `pyserial` enumerates). Scripted route: 10 s moving at 50 km/h → 4 s multipath glitch (0.5 km/h but +200 m) → 79 s parked → 4 s teleport (+5000 m) → 40 s moving. Results: **multipath glitch rejected** and **teleport rejected** (max eastward offset 694 m vs the 5000 m the teleport would have added); **the 79 s park added no fake distance**; **the service was restarted mid-recording and the buffer survived intact — 52 points before and after, and it kept recording afterwards**; save produced track 69 (52 pts, 693.7 m, 318 s) via the shared insert path. Test track deleted, GPS config restored to `auto`, fake source killed — **63 tracks before and after** (62 real + demo track 68).
*What the test exposed.* Stop detection returned 0 stops for the 80 s halt. Root-caused to the stationary gate leaving a stale `speed` on the timestamp-refreshed point — a **pre-existing bug**, identical in the old JS, not introduced here. Documented in Pending with a proposed fix; **not applied**, because it changes recorded data semantics.
*Also.* The repo's `ops-toc.service` still said `ExecStart=/usr/bin/python3` while the installed unit on CD runs the venv interpreter — repo copy corrected to match reality. `py_compile` + `node --check` clean.
