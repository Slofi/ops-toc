...
| **Depends on** | OM shared log DB (toc_log)<br>mbtileserver :8092 (shared tiles)<br>[[map-tiles-overview]] — CARTO basemaps need a key on the tile URL since 2026-08-26 (keyless = watermarked tiles, HTTP 200). **⚠️ The app ships NO key (2026-09-25): `/api/tile-layers` serves `?key={cartokey}` and `resolveTileUrl()` fills it from this browser's `cartoApiKey` (Map Keys → CARTO API key). Without one the CARTO layers load watermarked, and an offline download of them is refused.** |
| **Updated** | 2026-09-25 (DeepSeek — **the shipped CARTO key is gone**: `{cartokey}` placeholder + a Map Keys field; plus an offline-download guard so a keyless download can't be baked into an `.mbtiles`) · 2026-08-11 (S404 — **stop detection fixed**: halts bracketed by two at-rest markers, verified live on CD, 79 s park → 1 stop of 79 s, distance unchanged at 693.7 m) · 2026-08-10 (server-side recorder landed & verified on CD) |
